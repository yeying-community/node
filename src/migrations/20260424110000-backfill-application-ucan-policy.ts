...
function resolvePolicy(
  ...
  if (!targetUrl && String(app.code || '').trim() === CHAT_CODE) {
    const router = findRouterTarget(apps, app.uid);
    if (router) {
      targetUrl = parseLocationUrl(router.location);
    }
  }
  ...
  const audience = buildAudienceFromUrl(targetUrl);
...
export class BackfillApplicationUcanPolicy20260424110000 implements MigrationInterface {
  name = 'BackfillApplicationUcanPolicy20260424110000';

  async up(queryRunner: QueryRunner): Promise<void> {
    const dbType = queryRunner.connection.options.type;
    if (dbType !== 'postgres') {
      throw new Error(
        `BackfillApplicationUcanPolicy20260424110000 only supports postgres, got ${dbType}`
      );
    }
