			...
			try {
				function __body() {
					...
					if (__mimeType === 'application/json') {
						return v.modelApiMiniosdkPresignedUploadUrlRequestFromRequest('body', req.body)
					}
					console.error(`Invalid request content type: ${__contentType}`)
					throw new Error(`Invalid request content type: ${__contentType}`)
				}

				impl.minioPresignedUploadUrl(__body()).then(function (response) {
					if (response.status === 200) {
						let body: any
						try {
							body = v.modelApiMiniosdkPresignedUploadUrlResponseToResponse('response', response.body)
						} catch (error) {
							console.error('Invalid response body in minio.minioPresignedUploadUrl', error)
							res.status(500)
							res.send()
							return
						}
					...
					console.error('Unexpected error in minio.minioPresignedUploadUrl', error.stack || error)
					res.status(500)
					res.send()
				})
			} catch (error) {
