...
function sendError(res: Response, error: unknown) {
  const status = error instanceof ScopedGrantError ? error.status : 500
...
export function registerPublicAuthGrantRoutes(app: Express) {
  ...
  app.post(BASE_PATH, async (req, res) => { try { const subjectId = subjectFromRequest(req); res.json(ok(await service.create({ subjectId, appId: req.body?.appId, audience: req.body?.audience, capabilities: req.body?.capabilities, expiresAt: req.body?.expiresAt }))) } catch (error) { sendError(res, error) } })
  ...
  app.post(`${BASE_PATH}/:grantId/revoke`, async (req, res) => { try { res.json(ok(await service.revoke({ grantId: req.params.grantId, subjectId: subjectFromRequest(req), reason: req.body?.reason }))) } catch (error) { sendError(res, error) } })
