<template>
  <div class="passkey-history">
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>{{ label('动态验证码测试历史', 'TOTP Test History') }}</el-breadcrumb-item>
    </el-breadcrumb>

    <div class="history-head">
      <div>
        <div class="history-title">{{ label('动态验证码测试历史', 'TOTP Test History') }}</div>
        <div class="history-subtitle">{{ label('查看最近的动态验证码登录测试记录和步骤详情。', 'Review recent TOTP login test records and step details.') }}</div>
      </div>
      <div class="history-actions">
        <el-button @click="goBack">{{ label('返回动态验证码', 'Back') }}</el-button>
...
const records = ref<TotpTestHistoryRecord[]>([]);
...
function actionLabel(action: string) {
  const map: Record<string, string> = {
    ...
    approve_totp: label('动态验证码确认', 'Approve with TOTP'),
...
function goBack() {
  router.push('/market/dev/my-config/?authTab=totp').catch(() => undefined);
}
