...
| Notification（设计草案） | `/api/v1/public/*` + `/api/v1/admin/*` | 用户收件箱、SSE、Webhook 配置、重试与广播 |
...
- **Auth**
  ...
  - `POST /api/v1/public/auth/passport/bind/request`（Bearer JWT/UCAN，创建或复用 `subjectId -> wallet` 绑定）
  - `POST /api/v1/public/auth/passport/bind/confirm`（Bearer JWT/UCAN，确认钱包绑定证明）
  - `GET /api/v1/public/auth/passport/bindings`（Bearer JWT/UCAN）
  - `POST /api/v1/public/auth/passport/passkey/register/request`（Bearer JWT/UCAN，生成 subject 级 WebAuthn registration options）
  - `POST /api/v1/public/auth/passport/passkey/register/confirm`（Bearer JWT/UCAN，确认并持久化 subject 级 credential）
  - `GET /api/v1/public/auth/passport/passkey/credentials`（Bearer JWT/UCAN，列出 subject 级 credential）
  - `POST /api/v1/public/auth/passport/passkey/credentials/revoke`（Bearer JWT/UCAN，撤销 subject 级 credential）
  - `POST /api/v1/public/auth/passport/authorize/request`（appId + redirectUri + codeChallenge）
  - `GET /api/v1/public/auth/passport/authorize/request/:requestId`
  - `POST /api/v1/public/auth/passport/authorize/challenge`（requestId，生成 WebAuthn assertion options）
  - `POST /api/v1/public/auth/passport/authorize/approve`（Bearer JWT/UCAN + requestId，或 passkeyRequestId + credential）
  - `POST /api/v1/public/auth/passport/authorize/exchange`（code + appId + redirectUri + codeVerifier）
  - 约束：
    - `appId` 必须是已上架应用的 `AppId`（`applications.uid`）
    - `redirectUri` 必须命中应用发布字段 `redirectUris`
    - `authorize/request` 返回 `verifyUrl`，二维码和手机相机扫码应打开 `/passport/authorize?requestId=...`
    - Passport 授权码必须使用 PKCE S256，`exchange` 会校验 `codeVerifier`
...
- 通知中心当前已实现一期“站内通知 + SSE”最小能力；Webhook、广播、重试、通知详情等仍保留为后续阶段。
