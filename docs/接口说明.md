...
- **Auth**
  ...
  - `POST /api/v1/public/auth/totp/authorize/request`（address + clientId + redirectUri）
  ...
  - `POST /api/v1/public/auth/totp/authorize/exchange`（code + clientId + redirectUri）
  - 约束：
    - `clientId` 必须是已上架应用的 `AppId`（`applications.uid`）
...
- **Applications**
  ...
  - 关键字段：
    - `redirectUris`（可选，数组；建议在发布时配置，供中心化 UCAN `authorize` 使用）
- **Audits**
