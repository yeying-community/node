...
- **Auth**
  ...
  - `POST /api/v1/public/auth/mobile/authorize/request`（address + clientId + redirectUri）
  - `GET /api/v1/public/auth/mobile/authorize/request/:requestId`
  - `POST /api/v1/public/auth/mobile/authorize/approve`（requestId + TOTP code）
  - `POST /api/v1/public/auth/mobile/authorize/exchange`（code + clientId + redirectUri）
- **Health**
