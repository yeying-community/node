...
- 允许两种 token：
  ...
  - **UCAN**（需满足 `aud` + `cap`）
- UCAN 支持两种来源：
  - **wallet_ucan**：钱包签发（proof chain）
  - **central_ucan**：Node 统一 Issuer 签发
...
- `exp` 未过期
- `wallet_ucan` 要求证明链有效
- `central_ucan` 要求 issuer 命中 Node 统一 Issuer DID 且模式允许（`issue`/`hybrid`）
...
- 历史 `UNKNOWN` 账号在再次登录/命中活跃校验时会自动升级为 `NORMAL`

## 当前实际授权边界（现状）
- **已生效**：Token 必须通过（JWT/UCAN）。
- **已生效**：缺失 `user_state` 的用户会被显式补齐为 `NORMAL + ACTIVE`。
- **已生效**：资源归属校验（应用创建、更新、删除、上架/下架仅 owner 可操作；管理员可覆盖）。
- **已生效**：应用/配置/审核申请等业务写接口要求角色为 `NORMAL` 或 `OWNER`。
- **已生效**：MPC 写接口（创建会话/加入会话/发送消息）要求角色为 `NORMAL` 或 `OWNER`。
- **已生效**：审核权限（仅审批人可 approve/reject）。
- **已生效**：approve/reject 额外要求治理角色（`OWNER` 或 vault `ADMIN_DIDS` 白名单）。
- **已生效**：多审批人阈值（`audit.approvers` + `audit.requiredApprovals`），达到阈值才通过；任一驳回即拒绝。
- **已生效**：用户冻结/禁用限制（冻结/禁用用户禁止发布/审批）。
- **已生效**：管理员前缀校验（`/api/v1/admin/*`，角色 `OWNER` 或 vault `ADMIN_DIDS` 白名单）。
...
- **已生效**：市场应用、审核、MPC、管理员用户状态、管理员发布和邮件管理等人工敏感写操作启用钱包动作签名校验。
...
本协议用于 EVM 地址主体的人工业务动作，不是 SIWE 登录消息，也不用于 Wallet Identity controller 操作。Wallet Identity 的 Passkey/TOTP 管理使用 [登录授权](./登录授权.md#钱包身份敏感操作授权) 定义的 Ed25519 一次性动作授权。Pusher 事件发布、Runtime Agent 等机器调用使用各自的服务密钥签名，不弹出个人钱包。
