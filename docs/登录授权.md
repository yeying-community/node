...
sequenceDiagram
  ...
  U ->> PC: 钱包 SIWE 登录
  PC ->> BE: GET /auth/mobile/totp/provision (Bearer JWT)
  BE -->> PC: otpauthUri/secret（用于认证器绑定）
  PC ->> BE: POST /auth/mobile/bind/request (Bearer JWT)
  BE -->> PC: requestId + verifyUrl
  U ->> M: 扫码/打开 verifyUrl
  M ->> BE: GET /auth/mobile/bind/request/:requestId
  M ->> BE: POST /auth/mobile/bind/approve { requestId, code }
...
sequenceDiagram
  ...
  U ->> C: 输入 address，点击登录
  C ->> N: POST /auth/mobile/authorize/request { address, clientId, redirectUri, state }
  ...
  C ->> N: POST /auth/mobile/authorize/exchange { code, clientId, redirectUri }
  N -->> C: JWT + UCAN
...
- Node 前端提供公开页面：`/mobile-auth?requestId=...`（无需钱包会话）
- 页面职责：
  - 查询授权请求：`GET /api/v1/public/auth/mobile/authorize/request/:requestId`
  - 提交验证码授权：`POST /api/v1/public/auth/mobile/authorize/approve`
  - 授权成功后按返回的 `redirectTo` 自动回跳 Chat

关键约束：
- `clientId + redirectUri` 必须命中以下任一规则：
  - `mobileAuth.clients` 显式白名单。
  - `clientId=AppId(applications.uid)` 且应用已发布时的动态回跳白名单。
- 推荐将 `clientId` 直接使用应用市场 `AppId`（`applications.uid`）。
- 当 `clientId` 命中已发布应用（`is_online=true`）时，Node 可按应用 `location` 自动推导回跳白名单：
  - `https://<app-origin>/central-ucan-callback.html`
  - `https://<app-origin>/chat-callback.html`
- 若未命中已发布应用，则回退到 `mobileAuth.clients` 白名单校验。
...
- 中心化签发接口：
  ...
  - `GET /api/v1/public/auth/mobile/status`
  - `GET /api/v1/public/auth/mobile/totp/provision`（Bearer JWT/UCAN）
  - `POST /api/v1/public/auth/mobile/bind/request`（Bearer JWT/UCAN）
  - `GET /api/v1/public/auth/mobile/bind/request/:requestId`
  - `POST /api/v1/public/auth/mobile/bind/approve`
  - `POST /api/v1/public/auth/mobile/authorize/request`
  - `GET /api/v1/public/auth/mobile/authorize/request/:requestId`
  - `POST /api/v1/public/auth/mobile/authorize/approve`
  - `POST /api/v1/public/auth/mobile/authorize/exchange`
