...
- `authorize/request` 签发策略固定读取应用发布字段：
  - `ucanAudience`（签发 aud）
  - `ucanCapabilities`（签发 capability 列表）
  - 任一缺失将拒绝授权：`TOTP_AUTH_APP_POLICY_NOT_CONFIGURED`
...
- `UCAN audience mismatch`
  ...
  - 检查：
    ...
    - 中心化模式：应用发布配置 `ucanAudience` 与目标后端期望 aud（例如 Router 的 `did:web:127.0.0.1:3011`）
...
- `TOTP_AUTH_APP_POLICY_NOT_CONFIGURED`
  - 含义：应用未配置中心化签发策略（`ucanAudience` 或 `ucanCapabilities` 为空）
  - 处理：在应用发布/编辑页补齐上述两个字段后重试授权
