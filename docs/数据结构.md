...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| uid | uuid PK | 当前身份授权使用的稳定应用 AppId |
| did | varchar(128) | 现有应用/资源 DID，兼容查询和审核标识；当前不等同于独立 appDid |
| version | int | 版本 |
...
- `redirect_uris` 存储完整回跳地址列表（可为 HTTPS/HTTP 或已明确登记的应用协议，如 `chat://localhost/...`），单地址历史记录仍是纯文本，多地址记录使用 JSON 数组字符串；服务端按精确匹配校验 `redirectUri`。
- 应用空间、授权码和刷新会话使用稳定的 `uid/appId`。`applications` 保存当前版本快照，完整版本历史写入 `application_releases`。
- `did` 继续作为现有应用/资源兼容标识；当前没有独立 `appDid`、verification method 或应用制品验签闭环。`signature` 不能被解释为 appDid 签名。

## application_releases

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| uid | uuid PK | 版本记录内部标识 |
| application_uid | varchar(64) | 稳定 AppId，关联 `applications.uid` |
| version | int | 当前兼容版本号 |
| metadata_json | text | 该版本的应用元数据快照 |
| release_digest | varchar(128) | 制品摘要；完整制品验签接入前可为空 |
| signature | text | 兼容签名字段；当前不代表独立 appDid 签名 |
| status | varchar(64) | `draft/published/withdrawn` |
| created_at | varchar(64) | 创建时间 |
| updated_at | varchar(64) | 更新时间 |
...
```text
unique(applicationUid, version)
```

首次注册会同时创建应用当前快照和 release；携带已有 `uid` 发布更高版本时复用原 AppId，事务内保存旧版本快照、新 release 和新的当前快照。同一 AppId 不允许更换 owner 或现有 `did`，也不允许覆盖既有版本。已上线应用审核新版本时，应用级 `is_online` 保持为真，release 独立使用 `draft/reviewing` 状态，避免现有登录授权在审核期间中断。独立 `appDid` 和应用制品签名暂不属于当前数据模型。
...
| application_did | varchar(128) | 现有应用/资源 DID（兼容字段，不是独立 appDid） |
