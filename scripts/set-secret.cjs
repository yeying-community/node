...
async function run() {
  ...
  if (options.help || !options.name) {
    process.stdout.write('Usage: node scripts/set-secret.cjs KEY [--file run/secrets.enc.json]\n');
    return;
  }
  ...
  const value = await readPassword({ promptText: `请输入 ${name} 的值` });
  if (!value) throw new Error('密钥值不能为空');
  secrets[name] = value;
  replaceVault(options.file, encryptSecrets(secrets, password));
  process.stdout.write(`Updated secret: ${name}\n`);
}
