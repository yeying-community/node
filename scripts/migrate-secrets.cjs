...
const LEGACY = ['UCAN_ISSUER_PRIVATE_KEY', 'JWT_SECRET', 'PASSPORT_ASSERTION_SECRET', 'NOTIFICATION_WEBHOOK_MASTER_KEY'];
...
async function run() {
  if (process.argv.includes('--help') || process.argv.includes('-h')) {
    process.stdout.write('Usage: node scripts/migrate-secrets.cjs\nMigrates legacy vault keys and re-encrypts Webhook records for the unified key model.\n');
    return;
  }
  ...
  const legacyWebhook = secrets.NOTIFICATION_WEBHOOK_MASTER_KEY;
  if (legacyWebhook) {
    ...
    await client.connect(); try { await client.query('BEGIN'); const schema = config.database.schema || 'public';
      ...
      await client.query('COMMIT'); process.stdout.write(`Re-encrypted records: webhook=${totals.webhook}\n`);
